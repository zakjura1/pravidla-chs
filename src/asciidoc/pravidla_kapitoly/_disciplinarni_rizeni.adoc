...
. [[dru-2,{counter:dru}]]Hlavní rozhodčí má oprávnění přijmout následující opatření v případě porušení pravidel soutěže, co se týče nekázně kteréhokoliv <<#clen-tymu,člena týmu>>:
...
=== DISKVALIFIKACE (DSQ)
. Hlavní rozhodčí je oprávněný diskvalifikovat libovolného <<#clen-tymu,člena týmu>>. 
...
. Hlavní rozhodčí je oprávněný diskvalifikovat za chování libovolného <<#clen-tymu,člena týmu>>. Jakákoliv diskvalifikace za chování bude potrestána udělením červené karty a bude postoupena Disciplinární komisi ČHS. A dále okamžitou diskvalifikací soutěžícího ze všech soutěží dané <<#soutez,Soutěže>>. 
