...
. V souladu s částí <<#prohlidka-cesty>> obecných pravidel, soutěžící (jako skupina) mají povolenou prohlídku cest, které polezou za podmínek izolace.
...
. Nákres cesty musí být připravený Hlavním stavěčem za konzultace s Hlavním rozhodčím před startem každého kola soutěže.
...
. Hodnocení pokusu soutěžícího bude pozastaveno na posledním chytu v nákresu, o kterém si Hlavní stavěč myslí, že je poslední, ze kterého je možné zapnout nezapnutou expresku a soutěžící tento chyt drží nebo jej mine. A to až do okamžiku, kdy je tato expreska zapnuta.
